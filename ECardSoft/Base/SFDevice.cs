...
namespace ECard78
{
  ...
  public class SFReadData
  {
        ...
        for (int j = 0; j < logCount; j++)
        {
          ...
          if (DeviceObject.objKS.FeeLogDataValue(j, ref feeLog))
          {
            ...
            if (feeLog.ReadMark == 2)
            {
              ...
              RecMoney += Math.Abs(feeLog.SFAmount);
              aMoney += Math.Abs(feeLog.SFAmount);
            }
            else if (feeLog.ReadMark == 3)
            {
              ...
              RecMoney += Math.Abs(feeLog.SFAmount);
              rMoney += Math.Abs(feeLog.SFAmount);
            }
            else if (feeLog.ReadMark == 0xf0)
            {
              ...
              RecMoney += Math.Abs(feeLog.SFAmount);
              fMoney += Math.Abs(feeLog.SFAmount);
            }
            else if (feeLog.ReadMark != 0xff)
            {
              ...
              RecMoney += Math.Abs(feeLog.SFAmount);
              nMoney += Math.Abs(feeLog.SFAmount);
            }
        ...
        for (int j = 0; j < logCount; j++)
        {
          if (DeviceObject.objKS.FeeLogDataValue(j, ref feeLog))
          {
            ...
            else if (feeLog.ReadMark != 0xff)
            {
              ...
              if (feeLog.IsCount == 1)
              {
                Money = Money + Math.Abs(feeLog.SFAmount);
                RecordCount++;
    ...
    {
      string ret = "";
      if (feeLog.SFType == 3 || (feeLog.SFType == 200 && feeLog.ProductID[0] > 0))
      {
        for (int i = 0; i < 10; i++)
        {
          if (SystemInfo.IsDecimalProduct)
            ...
            ret += feeLog.ProductID[i].ToString("0000") + feeLog.ProductNum[i].ToString("000");
        }
      }
      return ret;
    }
    ...
    private void WriteTextFile(TFeeLog feeLog, int MacSN, int MacType, bool IsNew)
    {
      ...
      string msg = MacSN.ToString("00000") + Sep + MacType.ToString("00") + Sep +
        ...
        feeLog.MealType.ToString() + Sep + feeLog.SFAmount.ToString("0.00") + Sep +
        feeLog.CardBalance.ToString("0.00") + Sep + feeLog.SFDate.ToString(SystemInfo.SQLDateTimeFMT) + Sep +
        feeLog.CardUseTimes.ToString() + Sep + feeLog.OpterNo.ToString() + Sep + OrderDate + Sep + Product + 
        Sep + feeLog.TradeNo;
      Pub.WriteTextFile(fileName, msg);
    }

    private string GetSFSQL(TFeeLog feeLog, int MacSN, bool IsNew)
    {
      ...
      return Pub.GetSQL(DBCode.DB_004005, new string[] { val, feeLog.CardID, feeLog.SFType.ToString(), 
        feeLog.SFDate.ToString(SystemInfo.SQLDateTimeFMT), feeLog.SFAmount.ToString(), feeLog.CardBalance.ToString(), 
    ...
    private string GetSFMobileSQL(TFeeLog feeLog, int MacSN)
    {
      ...
      return Pub.GetSQL(DBCode.DB_004005, new string[] { "10", (feeLog.SFType - 200).ToString(), 
        feeLog.SFDate.ToString(SystemInfo.SQLDateTimeFMT), feeLog.SFAmount.ToString(), MacSN.ToString(), 
        Product, feeLog.TradeNo, OprtInfo.OprtSysID});
    }

    private string GetSFOrderSQL(TFeeLog feeLog, int MacSN, int ReadMark)
    {
      return Pub.GetSQL(DBCode.DB_004005, new string[] { "9", feeLog.CardID, feeLog.SFType.ToString(), 
        feeLog.SFDate.ToString(SystemInfo.SQLDateTimeFMT), feeLog.SFAmount.ToString(), feeLog.CardBalance.ToString(), 
        feeLog.MealType.ToString(), MacSN.ToString(), feeLog.OpterNo.ToString(), feeLog.CardUseTimes.ToString(), 
        OprtInfo.OprtSysID, feeLog.PhyID, feeLog.MacTAG, ReadMark.ToString() });
    }
    ...
    private void SaveDB(Database db, TFeeLog feeLog, int MacSN, int MacType, bool IsNew)
    {
      ...
      if (MacType == 36)
      {
        if (feeLog.SFAmount != 0) sql.Add(GetSFSQL(feeLog, MacSN, IsNew));
      ...
      else
      {
        if (feeLog.ReadMark == 0xF0)
          sql.Add(GetSFMobileFailSQL(feeLog, MacSN));
        else if (feeLog.SFType == 100)
          sql.Add(Pub.GetSQL(DBCode.DB_005001, new string[] { "0", feeLog.CardID, "20", 
            feeLog.SFDate.ToString(SystemInfo.SQLDateTimeFMT), Math.Abs(feeLog.SFAmount).ToString(), 
            feeLog.CardBalance.ToString(), OprtInfo.OprtSysID }));
        else if (feeLog.SFType == 8 && feeLog.SFAmount == 0)
          sql.Add(GetSFOrderSQL(feeLog, MacSN, 0));
        else if (feeLog.SFType == 200 || feeLog.SFType == 201)
          sql.Add(GetSFMobileSQL(feeLog, MacSN));
        else
          sql.Add(GetSFSQL(feeLog, MacSN, IsNew));
      }
      db.ExecSQL(sql);
    }

    private string GetSFErrorSQL(TFeeLog feeLog, int MacSN, bool IsNew)
    {
      ...
      return Pub.GetSQL(DBCode.DB_004005, new string[] { val, feeLog.CardID, feeLog.SFType.ToString(), 
        feeLog.SFDate.ToString(SystemInfo.SQLDateTimeFMT), feeLog.SFAmount.ToString(), feeLog.CardBalance.ToString(), 
        feeLog.MealType.ToString(), MacSN.ToString(), feeLog.OpterNo.ToString(), feeLog.CardUseTimes.ToString(), 
        OprtInfo.OprtSysID, Product, OrderDate, feeLog.PhyID, feeLog.MacTAG, feeLog.TradeNo, JSStartTime, JSEndTime, JSCount });
    }

    private string GetSFReturnSQL(TFeeLog feeLog, int MacSN, bool IsNew)
    {
    ...
    private void SaveErrorDB(Database db, TFeeLog feeLog, int MacSN, int MacType, bool IsNew)
    {
      ...
      if (MacType == 36)
      {
        if (feeLog.SFAmount != 0) sql.Add(GetSFErrorSQL(feeLog, MacSN, IsNew));
      }
      else
      {
        if (feeLog.SFType == 100)
          ...
          sql.Add(GetSFOrderSQL(feeLog, MacSN, 1));
        else
          sql.Add(GetSFErrorSQL(feeLog, MacSN, IsNew));
      }
    ...
    private void SaveReturnDB(Database db, TFeeLog feeLog, int MacSN, int MacType, bool IsNew)
    {
      List<string> sql = new List<string>();
      sql.Clear();
      sql.Add(GetSFReturnSQL(feeLog, MacSN, IsNew));
      db.ExecSQL(sql);
    }

    private string GetSFMobileFailSQL(TFeeLog feeLog, int MacSN)
    {
      string Product = GetProduct(feeLog);
      return Pub.GetSQL(DBCode.DB_004005, new string[] { "15", feeLog.CardID, feeLog.SFType.ToString(), 
        ...
        feeLog.TradeNo, OprtInfo.OprtSysID});
    }
