...
namespace ECard78
{
    ...
    public class MJReadData
    {
        ...
        public bool ReadDataNew(Database db, string MacSN, ref int RecordCount, ref int RecordIndex, ProcessReadData prog)
        {
           ...
           // bool ret = false;
            ...
            if (!IsStop)
            {
            //DeviceObject.objMJNew.ClearNewLog();
             RetryClear:
                bool ret = DeviceObject.objMJNew.ClearNewLog();
                if (!ret)
                {
    ...
    public class AccessV2API
    {
        ...
        public bool RealReadSetting(uint id, out TYPE_Setting setting)
        {
            setting = new TYPE_Setting();
            Succeed = AccessV2_GetDeviceInfo(id, ref setting);
            return Succeed != 0;
        }
        ...
        public int realReadBlockLog(uint id, UInt32 block, ref TYPE_LogPack pack)
        {
            return AccessV2_GetBlockLogNew(id, block, ref pack);
        }

        [DllImport("NBSDK.dll", EntryPoint = "AccessV2_ClrLogCount")]
        private static extern int AccessV2_ClrLogCount(UInt32 devNum);
        public bool ClearNewLog()
        {
            Succeed = AccessV2_ClrLogCount(DeviceID);
            return Succeed != 0;
        }

        public bool realClearNewLog(uint id)
        {
            Succeed = AccessV2_ClrLogCount(id);
            return Succeed != 0;
        }
