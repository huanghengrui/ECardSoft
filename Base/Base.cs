...
namespace ECard78
{
    public struct SystemInfo
    {
        ...
        public static string SQLDateHMS = "HH:mm:ss";
        ...
        public static int CardValidDays = 360;
        ...
        public static bool HasSEA = false; //海系列
        ...
        public static bool UbackEmp = true;
        public static bool DbackEmp = true;
        public static bool Urealback = true;
        public const int MaxTimeNo = 99;
        public const int MaxTimeNoNew = 50;
        public const int WanInterval = 1000;
        public static int GetDataCount = 0;
        ...
        public static bool HasForcedRecoveryOfData = true;
        public static int SubsidyRestrictionFlag = 0;
        ...
        public static bool IsGetDataPhoto = false;
        ...
        public static bool IsOperationCard = false;
        public static bool IsDimission = false;
        public static bool IsDelDimission = false;
    ...
    public class Base
    {
        ...
        public bool MessageBoxShowData(string Msg, MessageBoxIcon Icon)
        {
            string Title = SystemInfo.AppTitle;
            if (Title == "") Title = GetWindowTitle().ToString();
            return MessageBox.Show(Msg, Title, MessageBoxButtons.RetryCancel, Icon,
        ...
        public bool CheckTextMaxLength(string LabelText, string Text, int MaxLength)
        {
            ...
            if (!ret) ShowErrorMsg(string.Format(GetResText("Public", "ErrorOutrideMaxLength", ""), size, MaxLength));
            return ret;
        }
        ...
        public TConnInfoSEA ValueToConnInfoSEA(string MacSN, string MacIPAddress,
        string MacPort, string MacConnName,string MacConnPWD)
        {
            TConnInfoSEA connInfo = new TConnInfoSEA();
            connInfo.NetIP = MacIPAddress;
            connInfo.NetPort = Convert.ToInt32(MacPort);
            connInfo.NetName = MacConnName;
            connInfo.NetPassword = MacConnPWD;
            connInfo.MacSN = MacSN;
            return connInfo;
        }
        ...
        public bool CheckCardExists(bool ShowError)
        {
            ...
            {
                if (ShowError) ShowErrorMsg(GetResText("", "ReadCardError3", ""));
                return false;
            }
            return true;
        }
        ...
        public bool CheckCardExists(ref string CardData10, ref string CardDataH, ref string CardData8, bool ShowError)
        {
            ...
            {
                if (ShowError) ShowErrorMsg(GetResText("", "ReadCardError1", ""));
                return false;
            }
        ...
        public bool ReadCardInfo(ref HSUNFK.TCardPubData pubData, ref HSUNFK.TCardSFData sfData)
        {
            ...
            {
                ShowErrorMsg(GetResText("", "ErrorCardkey", ""));
                return false;
            }
            byte DataFlag = 0;
            ...
            if (cardRet != 0)
            {
                ShowErrorMsg(GetCardMsg(cardRet));
                return false;
            }
            if ((DataFlag != 1) && (DataFlag != 2) && (DataFlag != 3))
            {
                ShowErrorMsg(GetResText("", "ErrorCardInfoChaos", ""));
                return false;
            }
            if(SystemInfo.AntiDuplication)
            {
                ...
                if (Sector != 0)
                {
                    ShowErrorMsg(GetResText("", "ReadCardError4", ""));
                    return false;
                }
            }
        ...
        public bool ShowSelectNoCardEmp(string Title, string OtherCoin, ref DataTable selList)
        {
            frmPubSelectNoCardEmp frm = new frmPubSelectNoCardEmp(Title, OtherCoin);
            bool ret = frm.ShowDialog() == DialogResult.OK;
            if (ret) selList = frm.dtData;
            return ret;
        }
        ...
        public bool CheckUseDate(DateTime ServerDate, DateTime UseDate)
        {
            ...
            {
                ShowErrorMsg(GetResText("Public", "MsgLastUseDateError", ""));
                return false;
            }
            return true;
        }
    ...
    public class Database
    {
      ...
      0x2E93, 0x3EB2, 0x0ED1, 0x1EF0
        ...
        public void SaveSnapLog(string MacSN, string PictureType, string OprtDate,string Temperature, string TemperatureAlarm, byte[] Photo)
        {
            string GUID = "";
            SaveSnapLog(MacSN, PictureType, OprtDate, Temperature, TemperatureAlarm, Photo, ref GUID);
        }
        public void SaveSnapLog(string MacSN, string PictureType, string OprtDate, string Temperature, string TemperatureAlarm, byte[] Photo , ref string GUID)
        {
            try
            {
                ...
                string sql = "select * from SEA_SnapShots where MacSN='" + MacSN + "' and OprtDate='" + OprtDate + "'";
                ...
                {
                    sql = "insert into SEA_SnapShots ([GUID], MacSN,PictureType,OprtDate,Temperature,TemperatureAlarm) VALUES(@GUID, @MacSN,@PictureType," +
            "@OprtDate,@Temperature,@TemperatureAlarm)";
                ...
                sqlCmd = new SqlCommand("insert into SEA_SnapShotsPhoto ([GUID],Photo ) VALUES(@GUID,@Photo)", sqlConn);
                sqlCmd.CommandTimeout = CommandTimeout;
        ...
        public bool IsGetDate()
        {
            if (SystemInfo.HasForcedRecoveryOfData)
            {
                ...
                {
                    //string tmp = string.Format(Pub.GetResText("RSEmp", "Error107", ""),oprt);
                    //Pub.MessageBoxShow(tmp,MessageBoxIcon.Warning);
                    return false;
                }
        ...
        public bool CheckCardExists(string CardSectorNo)
        {
            ...
            if (!ret) Pub.ShowErrorMsg(Pub.GetResText("", "ErrorIllegalCard", ""));
            return ret;
        }
        ...
        public bool EmpCardFa(string Title, string Oprt, string EmpSysID, string CardSectorNo, string CardData10,
          ...
          int CardStatusID, int OpterType, string PhyID, string MacTAG,ref int reStatus)
        {
            bool ret = false;
            DataTableReader dr = null;
        ...
        OpterType.ToString(), PhyID,  MacTAG});
            try
            {
                ...
                if (dr.Read())
                {
                    reStatus = Convert.ToInt32(dr["ReStatus"]);
                }
                ret = true;
        ...
        public bool EmpHistoryCard(string Title, string Oprt, string EmpSysID, string CardSectorNo, DateTime CardUseDate, int value)
        {
            bool ret = false;
            DataTableReader dr = null;
            string EmpNo = "";
            ...
            try
            {
                dr = GetDataReader(Pub.GetSQL(DBCode.DB_001003, new string[] { "247", EmpSysID, CardSectorNo }));
                while (dr.Read())
                {
                    OpterStartType = dr["OpterStartType"].ToString();
                    OpterEndType = dr["OpterEndType"].ToString();
                    if (OpterEndType == "" && value == 3)
                    {
                        sql = Pub.GetSQL(DBCode.DB_001003, new string[] { "248", Oprt, CardUseDate.ToString(SystemInfo.SQLDateTimeFMT), EmpSysID, CardSectorNo });
                        ExecSQL(sql);
                        goto END;
                    }
                }
                if (value == 3)
                    return ret;
                dr = GetDataReader(Pub.GetSQL(DBCode.DB_001003, new string[] { "215", CardSectorNo }));
                if (dr.Read())
                {
                    ...
                    CardTypeID = int.Parse(dr["CardTypeID"].ToString());
                    EmpCertNo = dr["EmpCertNo"].ToString();
                    EmpPhoneNo = dr["EmpPhoneNo"].ToString();
                }
            }
            catch (Exception E)
            {
            ...
            sql = "";
            sql = Pub.GetSQL(DBCode.DB_001003, new string[] { "246", EmpSysID,EmpNo,EmpName,
         DepartSysID,DepartID,DepartName, CardSectorNo,Oprt,CardUseDate.ToString(SystemInfo.SQLDateTimeFMT),
         CardTypeID.ToString(),CardTypeName,EmpCertNo,EmpPhoneNo});
            try
            {
                ExecSQL(sql);
                ret = true;
        ...
        public bool EmpCardModify(string Title, string Oprt, string CardSectorNo, int CardTypeID, string CardPWD,
          DateTime CardStartDate, DateTime CardEndDate, string EmpID, string EmpName, string DepartSysID)
        {
            bool ret = false;
        ...
        OprtInfo.OprtSysID, EmpID, EmpName,DepartSysID });
            try
            {
                ExecSQL(sql);
                ret = true;
        ...
        {
            bool ret = false;
            string sqll = Pub.GetSQL(DBCode.DB_001003, new string[] { "21", EmpSysID, PhyID });
        ...
        public void ReadSystemConfig()
        {
            ...
            //退卡、换卡之前的卡号有效天数
            SystemInfo.CardValidDays = ReadConfig("RS_System", "CardValidDays", 360);
            ...
            //启用防复制卡功能
            SystemInfo.AntiDuplication = ReadConfig("RS_System", "AntiDuplication", false);

            //启用强制回收数据才能对卡操作
            SystemInfo.HasForcedRecoveryOfData = ReadConfig("RS_System", "ForcedRecoveryOfData", true);

            //启用实时挂失
            SystemInfo.UbackEmp = ReadConfig("RS_System", "UbackEmp", true);

            //启用实时解挂
            SystemInfo.DbackEmp = ReadConfig("RS_System", "DbackEmp", true);

            //启用开启实时监控下传黑名单
            SystemInfo.Urealback = ReadConfig("RS_System", "Urealback", true);

            //启用补贴次数限制
            SystemInfo.SubsidyRestrictionFlag = ReadConfig("RS_System", "SubsidyRestriction", 0);
            //启用回收考勤记录带照片
            SystemInfo.IsGetDataPhoto = ReadConfig("KQ_System", "IsGetDataPhoto ", false);
        }
        ...
        public void UpdateEmpHistoryCard(string Title)
        {
            DataTableReader dr = null;
            DataTableReader drr = null;
            //DataTableReader dt = null;
            string EmpNo = "";
            string EmpName = "";
            string DepartSysID = "";
            string DepartID = "";
            ...
            //string CardEndDate = "";
            //int i = 0;
            string Oprt = Pub.GetResText("RSEmp", "ItemTAG1", "");
            string OprtEnd = Pub.GetResText("RSEmp", "ItemTAG5", "");
            try
            {
                dr = GetDataReader(Pub.GetSQL(DBCode.DB_001003, new string[] { "250" }));
                while (dr.Read())
                {
                    EmpNo = dr["EmpNo"].ToString();
                    EmpName = dr["EmpName"].ToString();
                    ...
                    CardTypeName = dr["CardTypeName"].ToString();
                    CardTypeID = int.Parse(dr["CardTypeID"].ToString());
                    CardUseDate = dr["FaDate"].ToString();
                    EmpSysID = dr["EmpSysID"].ToString();
                    CardSectorNo = dr["CardSectorNo"].ToString();
                    EmpCertNo = dr["EmpCertNo"].ToString();
                    EmpPhoneNo = dr["EmpPhoneNo"].ToString();

                    drr = GetDataReader(Pub.GetSQL(DBCode.DB_001003, new string[] { "247", EmpSysID, CardSectorNo }));
                    if (drr.Read())
                    {
                        continue;
                    }
                    ExecSQL(Pub.GetSQL(DBCode.DB_001003, new string[] { "246", EmpSysID,EmpNo,EmpName,
                    DepartSysID,DepartID,DepartName, CardSectorNo,Oprt,CardUseDate,
                    CardTypeID.ToString(),CardTypeName,EmpCertNo,EmpPhoneNo}));
                }
                dr.Close();
                //dr = GetDataReader(Pub.GetSQL(DBCode.DB_001003, new string[] { "250" }));
                ...
                //  EmpPhoneNo = dr["EmpPhoneNo"].ToString();
                //  i = 0;
                //  dt = GetDataReader(Pub.GetSQL(DBCode.DB_001003, new string[] { "253", EmpNo}));
                //  while(dt.Read())
                //  {
                //     CardSectorNo = dt["CardNo"].ToString();
                //     if(i==0)
                //           CardUseDate  = dt["OpterDate"].ToString();
                //     else if(i==1)
                //           CardEndDate  = dt["OpterDate"].ToString();
                //     i++;
                //  }
                //  dt.Close();
                //  drr = GetDataReader(Pub.GetSQL(DBCode.DB_001003, new string[] { "247", EmpSysID,CardSectorNo }));
                ...
                //  ExecSQL(Pub.GetSQL(DBCode.DB_001003, new string[] { "246", EmpSysID,EmpNo,EmpName, 
                //          DepartSysID,DepartID,DepartName, CardSectorNo,Oprt,CardUseDate,
                //          CardTypeID.ToString(),CardTypeName,EmpCertNo,EmpPhoneNo,OprtEnd,CardEndDate}));
                //}
                //dr.Close();
        ...
        public bool UpdateDatabase(string Title, DateTime dt)
        {
            ...
            {
                rm = new System.Resources.ResourceManager("ECard78.Properties.Resources", Assembly.GetExecutingAssembly());
                newDate = new DateTime(2020, 3, 5);
                if (SystemInfo.LastDBDate > newDate) newDate = SystemInfo.LastDBDate;
                if (dt < newDate)
                {
                    Pub.ShowMessageForm(Pub.GetResText("", "MsgUpgradeData", ""));
                    // UpdateEmpHistoryCard(Title);
                    for (int i = 1; i <= 29; i++)
                    {
        ...
        public bool CheckOprtRole(string SubID, string role, bool CheckForward, bool AllowCheckOprtRole)
        {
            ...
            {
                Pub.ShowErrorMsg(Pub.GetResText("", "ErrorIsForward", ""));
                return ret;
            }
        ...
        RoleEnd:
            if (!ret)
            {
                Pub.ShowErrorMsg(Pub.GetResText("", "ErrorNoPower" + role.ToUpper().Trim(), ""));
            }
            return ret;
        }
        ...
        public bool CheckDepartPowerByEmpSysID(string EmpSysID)
        {
            ...
            try
            {
                ...
                {
                    Pub.ShowErrorMsg(Pub.GetResText("", "ErrorNoDeptPower", ""));
                }
            }
            catch (Exception E)
            {
        ...
        public string GetDepartSysID(string DepartSysID)
        {
            string ret = "";
            string s = "";
        ...
        public string GetAutoEmpNo(UInt32 EnrollNumber)
        {
            ...
            try
            {
                while (ret == "")
                {
                    dr = GetDataReader(Pub.GetSQL(DBCode.DB_001003, new string[] { "2", s }));
                    if (dr.Read())
                    {
                        tmp++;
        ...
        public bool CheckDepositLimit(string EmpSysID, double LimitMoney, int LimitTimes, double Deposit)
        {
            ...
            try
            {
                ...
                if (dr.Read())
                {
                    ...
                    {
                        Pub.ShowErrorMsg(string.Format(Pub.GetResText("", "ErrorCardDepositLimit", ""), LimitMoney));
                    }
                    else if (LimitTimes > 0 && (i >= LimitTimes || i + 1 > LimitTimes))
                    {
                        Pub.ShowErrorMsg(string.Format(Pub.GetResText("", "ErrorCardDepositTimes", ""), LimitTimes));
                    }
                    else
                    {
                        ret = true;
